...
(function(root,factory){
  ...
  function sizes(model) {
    ...
    return new Map(model.shapes.map(s=>{
      ...
      const h=({cylinder:110,cone:68,frustum:82,pipe:95,sphere:r*2,hemisphere:r})[s.type];
      return [s.id,{r,bottom,h}];
    }));
  }
  ...
  function body(part) {
    ...
    let fill='',outline='';
    if(['cone','triangularPrism','pyramid'].includes(s.type)){fill=`M${-r} 0H${r}L0 ${h}Z`;outline=`M${-r} 0L0 ${h}L${r} 0`;}
    else if(s.type==='halfCylinder'){fill=`M${-r} 0H${r}A${r} ${r} 0 0 1 ${-r} 0Z`;outline=`M${-r} 0A${r} ${r} 0 0 0 ${r} 0`;}

(function () {
  ...
  function renderUnitGuide(ast,f) {
    ...
    $('#symbol-guide').innerHTML=`<div class="guide-heading"><h2>Størrelserne i formlen</h2><span>Din enhed → formlens enhed</span></div><p class="unit-intro">Vælg enhederne fra opgaven. Omregningerne indsættes i formlen, så du kan bruge de oplyste tal direkte.</p><dl class="symbol-list unit-symbol-list">${vars.map(v=>{
      const unit=E.inputUnit(model,v),expected=E.Units.base(v.dimension);
      return `<div><dt>${symbolHtml(v.symbol)}<span>${esc(v.label||E.DIMENSIONS[v.dimension].name)}</span></dt><dd><div class="unit-pair"><select class="unit-select" data-action="input-unit" data-id="${esc(E.Units.key(v))}" data-symbol="${esc(v.symbol)}" data-dimension="${esc(v.dimension)}" aria-label="Din enhed for ${esc(pretty(v.symbol))} · ${esc(E.DIMENSIONS[v.dimension].name)}">${unitOptions(v.dimension,unit.id)}</select><span aria-hidden="true">→</span><span>${esc(expected.label)}</span></div><span class="unit-operation">${esc(E.Units.hint(unit))}</span></dd></div>`;
    }).join('')}</dl><div class="result-unit-row"><div><label for="result-unit">Resultatets enhed</label><p>${symbolHtml(f.symbol)}<span>Vælg enheden for svaret</span></p></div><div><div class="unit-pair"><span>${esc(base.label)}</span><span aria-hidden="true">→</span><select id="result-unit" class="unit-select" data-action="result-unit" data-id="${esc(f.id)}" aria-label="Resultatets enhed">${unitOptions(f.dimension,output.id)}</select></div><span class="unit-operation">${esc(E.Units.hint(output,'fromBase'))}</span></div></div><p class="unit-footnote">Samme symbol bruger samme inputenhed i hele opsætningen. Resultatenheden vælges for hver formel. Valgene følger med, når du gemmer.</p>`;
  }
  ...
  document.addEventListener('change',event=>{
    ...
    if(a==='input-unit'){
      const variable={symbol:el.dataset.symbol,dimension:el.dataset.dimension},unit=E.Units.get(variable.dimension,el.value),key=E.Units.key(variable);
      model.inputUnits||={};
    ...
    }else if(a==='result-unit'){
      const f=model.formulas.find(f=>f.id===el.dataset.id);
      if(f){f.resultUnit=E.Units.get(f.dimension,el.value).id;expanded=true;save();render(true);}
    }else if(a==='source'){

(function () {
  ...
  function renderShapeInspector(s, index, ctx) {
    ...
    const inputs=Object.entries(type.inputs).map(([key,a])=>{
      ...
      const label=E.diameterKeys(s).includes(key)?a.label+' ('+(E.diameterSettings(s).basis==='outer'?'udvendig':'indvendig')+')':a.label+(key==='h'?' (indvendig)':'');

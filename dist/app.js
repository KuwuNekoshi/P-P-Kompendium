(function () {
  ...
  function formulaSummary(expression, dimension, ctx, lhs) {
    try { return E.math(ctx.expand(expression,dimension,'compact'),lhs,E.Units.base(dimension).label); }
  ...
  function renderPreview(ctx) {
    ...
    $('#formula-preview').innerHTML = `<div class="preview-header"><div><span class="eyebrow">DIN FORMEL</span><h2>${esc(f.name)}</h2></div><div class="segmented" role="group" aria-label="Udfoldning af formlen"><button data-action="expansion" data-expanded="false" aria-pressed="${!expanded}">Kort</button><button data-action="expansion" data-expanded="true" aria-pressed="${expanded}">Udfoldet</button></div></div>${rearrangeControl(f)}<div class="formula-display">${r.ok ? E.math(r.ast,f.symbol,E.resultUnit(f).label) : `<div class="error-box">${esc(r.error)}</div>`}</div><div class="preview-footer"><span>${expanded ? 'Formlerne er sat ind i hinanden' : 'Referencer vises som symboler'}</span><button class="text-button copy-button" data-action="copy-formula" ${r.ok ? '' : 'disabled'}>${icon('copy')} Kopiér formel</button></div>${note ? `<p class="formula-assumption">${esc(note)}</p>` : ''}${source?`<p class="formula-source">Skolens formelsamling · ${esc(source)}</p>`:''}`;
    let steps;
  ...
  function renderUnitGuide(ast,f) {
    ...
    $('#symbol-guide').innerHTML=`<div class="guide-heading"><h2>Størrelserne i formlen</h2><span>Tal og enheder fra opgaven</span></div><p class="unit-intro" id="value-input-help">Skriv eventuelt opgavens tal, fx 50 eller 2,5, og vælg enheden. Formlen viser enhed og symbol med småt efter tallet. Et tomt felt beholder symbolet. Nødvendige omregninger sættes ind og forkortes automatisk; selve resultatet beregner du på din lommeregner.</p><dl class="symbol-list unit-symbol-list">${vars.map((v,i)=>{
  ...
  function diameterEditor(s,index,ctx) {
    ...
    const lines=keys.map(key=>{
      ...
      return `<div class="diameter-equation">${result.ok?E.math(result.ast,d.symbol,E.Units.base(d.dimension).label):`<p class="inline-error">${esc(result.error)}</p>`}</div>`;
    }).join('');
  ...
  async function copyFormula() {
    ...
    const quantities=E.variables(r.ast).map(v=>v.symbol+': '+(v.inputValue!==undefined?v.inputValue.replace('.',',')+' ':'')+(v.reference?E.resultUnit(ctx.map.get(v.reference)):E.inputUnit(model,v)).label);
    const text=f.symbol+' ['+E.resultUnit(f).label+'] = '+E.plain(r.ast)+(quantities.length?'\n\nStørrelser og enheder:\n'+quantities.join('\n'):'');
  ...
  function finishTutorial() {
    ...
    tutorial = null;
    ({model,activeId,selected,expanded,geometryView,previewPose,libraryQuery,valueDrafts} = previous);
  ...
  function startTutorial() {
    if (tutorial) return;
    tutorial = {model,activeId,selected,view,expanded,geometryView,previewPose:preview3D?.getPose()||previewPose,libraryQuery,valueDrafts,

(function () {
  ...
  function renderUnitGuide(ast,f) {
    ...
    $('#symbol-guide').innerHTML=`<div class="guide-heading"><h2>Størrelserne i formlen</h2><span>Enheder fra opgaven</span></div><p class="unit-intro">Vælg enhederne fra opgaven. I fx kg/h vælger du tæller og nævner hver for sig. Nødvendige omregninger indsættes i formlen, og fælles faktorer forkortes væk. Brug de oplyste tal direkte.</p><dl class="symbol-list unit-symbol-list">${vars.map(v=>{
      const unit=E.inputUnit(model,v),expected=E.Units.base(v.dimension);
      return `<div><dt>${symbolHtml(v.symbol)}<span>${esc(v.label||E.DIMENSIONS[v.dimension].name)}</span></dt><dd><div class="unit-pair">${unitSelector(v.dimension,unit,'input-unit',E.Units.key(v),v.symbol)}<span class="unit-target"><span aria-hidden="true">→</span> SI: ${esc(expected.label)}</span></div><span class="unit-operation">SI-reference: ${esc(E.Units.hint(unit))}</span></dd></div>`;
    }).join('')}</dl><div class="result-unit-row"><div><label for="result-unit">Resultatets enhed</label><p>${symbolHtml(f.symbol)}<span>Vælg enheden for svaret</span></p></div><div><div class="unit-pair"><span class="unit-target">SI: ${esc(base.label)} <span aria-hidden="true">→</span></span>${unitSelector(f.dimension,output,'result-unit',f.id)}</div><span class="unit-operation">SI-reference: ${esc(E.Units.hint(output,'fromBase'))}</span></div></div><p class="unit-footnote">SI-referencerne viser grundomregningerne før forkortning. Følg den viste formel uden ekstra omregning. Samme symbol deler inputenhed i hele opsætningen; hver formel har sin egen resultatenhed. Valgene gemmes med opsætningen.</p>`;
  }
  ...
  async function copyFormula() {
    ...
    const quantities=E.variables(r.ast).map(v=>v.symbol+': '+(v.reference?E.resultUnit(ctx.map.get(v.reference)):E.inputUnit(model,v)).label);
    const text=f.symbol+' ['+E.resultUnit(f).label+'] = '+E.plain(r.ast)+(quantities.length?'\n\nIndsæt størrelserne i disse enheder:\n'+quantities.join('\n'):'');
  ...
  const tutorialSteps = [
    ...
    {section:'RESULTAT', title:'Se den færdige formel', target:'#formula-preview', body:'<p><strong>Kort</strong> viser referencer som symboler. <strong>Udfoldet</strong> sætter delformlerne ind i hinanden, som vist her. Unødige parenteser fjernes automatisk. Nødvendige parenteser bevares fx omkring en sum, der skal ganges, en samlet nævner i kopieret tekst eller et udtryk under en potens.</p><p><strong>Kopiér formel</strong> tager den viste formel og de valgte enheder med. Kompendiet hjælper dig med udtrykket; du indsætter opgavens tal, når du selv regner videre.</p>'},

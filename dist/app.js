(function () {
  ...
  function diameterEditor(s,index,ctx) {
    ...
    const lines=keys.map(key=>{
      ...
      return `<div class="diameter-equation">${result.ok?E.math(result.ast,d.symbol):`<p class="inline-error">${esc(result.error)}</p>`}</div>`;
    }).join('');
  ...
  document.addEventListener('click',event=>{
    ...
    else if(a==='new')replaceSetup({version:4,title:'Ny opsætning',tank:E.defaultTank(),shapes:[],formulas:[],connections:[]},'Start en tom opsætning?');

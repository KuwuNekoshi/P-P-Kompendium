(function () {
  ...
  const VALUE_DISPLAYS={
    ...
    units:{label:'Enheder',hint:'Viser de oprindelige symboler, fx B og L.'},
    both:{label:'Begge',hint:'Viser tal med enhed og symbol i mindre tekst.'}
  };
  let valueDisplay='both';
  try{const saved=localStorage.getItem(DISPLAY);if(Object.hasOwn(VALUE_DISPLAYS,saved))valueDisplay=saved;}catch(_){/* Storage is optional. */}
  const displayMath=(ast,lhs,unit)=>E.math(ast,lhs,unit,valueDisplay);
  ...
  function shapeIcon(type) {
    const drawings = {
      ...
      halfCylinder:'<path d="M5 25h26a13 13 0 0 1-26 0Zm0 0L20 10h26L31 25M46 10a13 13 0 0 1-13 13L18 38"/>',
      triangularPrism:'<path d="M5 24h27L18.5 44 5 24Zm0 0L20 8h27L32 24M47 8 33.5 28 18.5 44"/>',
      pyramid:'<path d="m5 17 27-9 15 10-27 9L5 17Zm0 0 21 28 21-27M20 27l6 18"/><path d="m32 8-6 37" stroke-dasharray="3 3"/>',
  ...
  function ownerTarget(path) { if(path[0]==='tank')return 'tank:thickness';if(path[0]==='shapes')return path[2]==='diameter'?`shape:${model.shapes[path[1]].id}:diameterThickness`:`shape:${model.shapes[path[1]].id}:input:${path[3]}`;return `formula:${model.formulas[path[1]].id}`; }
  ...
  function formulaSummary(expression, dimension, ctx, lhs) {
    try { return displayMath(ctx.expand(expression,dimension,'compact'),lhs,E.Units.base(dimension).label); }
    catch (error) { return `<p class="inline-error">${esc(error.message)}</p>`; }
  }
  function renderSidebar() {
    $('#library-total').textContent=Object.keys(E.BASE_FORMULAS).length;
  ...
  function renderFigures(ctx) {
    ...
    $('#construction-note').textContent = included.length ? 'Vælg en figur for at angive indre/ydre diameter og åbne/lukkede ender. Brug Sammensæt til at sætte en anden figur på en ende.' : 'Du kan også bruge formlerne uden figurer. Behold de størrelser, du allerede kender, som symboler.';
    ...
    const summaries=['volume','area'].map(dimension=>{
      try{return `<div class="assembly-formula"><div><span>${dimension==='volume'?'Samlet rumfang':'Samlet pladeareal'}</span><button class="text-button" data-action="use-assembly" data-dimension="${dimension}">Brug formel</button></div><div class="assembly-equation">${displayMath(ctx.expand(E.assembly(dimension),dimension),dimension==='volume'?'V_samlet':'A_plade',E.Units.base(dimension).label)}</div></div>`;}catch(error){return `<p class="inline-error">${esc(error.message)}</p>`;}
    }).join('');
    ...
    }).join('')}</div>`:'<p class="assembly-hint">Sæt fx en halvkugle på cylinderens bund. Fælles endeflader udelades automatisk fra overfladearealet.</p>'}${included.length?`<details class="assembly-totals" data-detail-key="assembly-totals"><summary>Rumfang og pladeareal for valgte beholderdele</summary>${summaries}<p class="field-note">Rumfang og pladeareal bruger indvendige mål. Åbne ender og samlinger er udeladt fra pladearealet. T9 er skolens tyndplademodel.</p></details>`:''}</section>` : '';
  ...
  function rearrangeControl(f) {
    if(f.expression.kind!=='formula')return '';
    const options=E.rearrangements(f.expression.formula),blocked=options.filter(o=>!o.available);
    const choices=options.filter(o=>o.id!==f.expression.formula).map(o=>`<option value="${esc(o.id||'')}" ${o.available?'':'disabled'}>${esc(pretty(o.symbol)+' · '+o.label)}${o.available?'':' · se forklaring nedenfor'}</option>`).join('');
    return `<div class="rearrange-control"><label for="isolate-quantity">Isolér en størrelse</label><select id="isolate-quantity" class="source-select" data-action="rearrange-formula" data-id="${esc(f.id)}"><option value="" selected>Vælg, hvad du vil finde…</option>${choices}</select><p>Opretter en ny formel med dine kendte størrelser og referencer. Den hidtidige venstreside bliver et kendt input.</p>${blocked.length?`<details><summary>Særligt om ${esc(blocked.map(o=>pretty(o.symbol)).join(' og '))}</summary><p>${esc(blocked[0].reason)}</p><button class="text-button" data-action="use-formula" data-formula="frustumMantleSlant">Brug kendt skrå højde</button></details>`:''}</div>`;
  }
  ...
  function renderResult(ctx,f){
    if(!resultVisible)return '';
    const expandedResult=ctx.safe('formula:'+f.id,'expanded',true);
    const result=expandedResult.ok?E.evaluate(expandedResult.ast):{status:'error',error:expandedResult.error};
    let body;
    if(result.status==='incomplete')body=`<p class="result-note">Udfyld de manglende værdier: ${result.missing.map(v=>symbolHtml(v.symbol)).join(', ')}</p>`;
    else if(result.status==='error')body=`<p class="inline-error">${esc(result.error)}</p>`;
    else{
      const magnitude=Math.abs(result.value),scientific=magnitude!==0&&(magnitude<.001||magnitude>=1e12);
      const number=new Intl.NumberFormat('da-DK',{minimumFractionDigits:3,maximumFractionDigits:3,useGrouping:false,notation:scientific?'scientific':'standard'}).format(result.value);
      body=`<output class="result-value">${symbolHtml(f.symbol)}<span>=</span><strong>${esc(number)}</strong><span>${esc(E.resultUnit(f).label)}</span></output><p class="result-note">Afrundet til 3 decimaler · følg formlens forudsætninger</p>`;
    }
  ...
  function unitSelector(dimension,unit,action,id,symbol='') {
    const ratio=E.Units.ratio(dimension),result=action==='result-unit';
    const label=result?'Resultatets enhed':`Din enhed for ${pretty(symbol)} · ${E.DIMENSIONS[dimension].name}`;
    const attributes=`data-id="${esc(id)}" data-dimension="${esc(dimension)}"${symbol?` data-symbol="${esc(symbol)}"`:''}`;
    if(!ratio)return `<div class="unit-choice"><select ${result?'id="result-unit"':''} class="unit-select" data-action="${action}" ${attributes} aria-label="${esc(label)}">${unitOptions(dimension,unit.id)}</select></div>`;
    const labelledPart=ratio.numerator.length>1?'numerator':'denominator';
    const selectPart=part=>{
      const options=ratio[part].map(u=>{
        const name=part==='denominator'&&ratio.groupDenominator?'('+u.label+')':u.id==='ton'?'t (ton)':u.label;
        return `<option value="${esc(u.id)}" ${u.id===unit.parts[part]?'selected':''}>${esc(name)}</option>`;
      }).join('');
      return `<select ${result&&part===labelledPart?'id="result-unit"':''} class="unit-select" data-action="${action}-part" ${attributes} data-part="${part}" aria-label="${esc(label)} · ${part==='numerator'?'tæller':'nævner'}" ${ratio[part].length===1?'disabled':''}>${options}</select>`;
    };
    return `<div class="unit-choice unit-fraction" role="group" aria-label="${esc(label)}">${selectPart('numerator')}<span class="unit-slash" aria-hidden="true">/</span>${selectPart('denominator')}</div>`;
  }
  function renderUnitGuide(ast,f) {
    const vars=E.variables(ast),output=E.resultUnit(f),base=E.Units.base(f.dimension);
    $('#symbol-guide').innerHTML=`<div class="guide-heading"><h2>Størrelserne i formlen</h2><span>Tal og enheder fra opgaven</span></div><p class="unit-intro" id="value-input-help">Skriv eventuelt opgavens tal, fx 50 eller 2,5, og vælg enheden. Med visningen Begge står enhed og symbol med småt efter tallet. Et tomt felt beholder symbolet. Nødvendige omregninger sættes ind og forkortes automatisk; i standardvisningen beregner du resultatet på din lommeregner.</p><dl class="symbol-list unit-symbol-list">${vars.map((v,i)=>{
      const unit=E.inputUnit(model,v),expected=E.Units.base(v.dimension),key=E.Units.key(v);
      const value=valueDrafts.get(key)??E.inputValue(model,v),invalid=E.parseInputValue(value)===null;
      return `<div><dt>${symbolHtml(v.symbol)}<span>${esc(v.label||E.DIMENSIONS[v.dimension].name)}</span></dt><dd><div class="value-entry"><label class="value-field"><span>Tal (valgfrit)</span><input type="text" inputmode="decimal" autocomplete="off" spellcheck="false" maxlength="32" data-action="input-value" data-id="${esc(key)}" value="${esc(value)}" placeholder="${esc(pretty(v.symbol))}" aria-label="${esc('Tal for '+v.symbol+' i '+unit.label+' (valgfrit)')}" aria-invalid="${invalid}" aria-describedby="value-input-help value-error-${i}"></label><button class="text-button clear-value" data-action="clear-value" data-id="${esc(key)}" aria-label="${esc('Ryd tallet for '+v.symbol+' og vis symbolet igen')}" ${value===''?'disabled':''}>Ryd</button></div><p class="value-error" id="value-error-${i}" ${invalid?'':'hidden'}>Skriv ét tal med komma eller punktum, fx −2,5. Ufuldstændige tal indsættes ikke.</p><div class="unit-pair">${unitSelector(v.dimension,unit,'input-unit',key,v.symbol)}<span class="unit-target"><span aria-hidden="true">→</span> SI: ${esc(expected.label)}</span></div><span class="unit-operation">SI-reference: ${esc(E.Units.hint(unit))}</span></dd></div>`;
    }).join('')}</dl><div class="result-unit-row"><div><label for="result-unit">Resultatets enhed</label><p>${symbolHtml(f.symbol)}<span>Vælg enheden for svaret</span></p></div><div><div class="unit-pair"><span class="unit-target">SI: ${esc(base.label)} <span aria-hidden="true">→</span></span>${unitSelector(f.dimension,output,'result-unit',f.id)}</div><span class="unit-operation">SI-reference: ${esc(E.Units.hint(output,'fromBase'))}</span></div></div><p class="unit-footnote">De små enheds- og symbolmærker forklarer tallet og skal ikke tastes på lommeregneren. Følg den viste formel uden ekstra omregning fra SI-referencerne. Samme symbol og størrelse deler tal og inputenhed i hele opsætningen. Skifter du enhed, beholdes det indtastede tal. Tal og enhedsvalg gemmes med opsætningen.</p>`;
  }
  function expressionEditor(expr, dimension, path, defaultSymbol, ctx, depth = 0) {
    ...
    if (expr.kind === 'symbol') html += `<div class="known-symbol">${symbolHtml(expr.symbol)}<span>Indtast evt. et tal under Størrelserne i formlen</span></div>`;
  ...
  function diameterEditor(s,index,ctx) {
    const keys=E.diameterKeys(s);if(!keys.length)return '';
    const settings=E.diameterSettings(s),sloped=E.slopedWall(s);
    const lines=keys.map(key=>{
      const d=ctx.map.get(`shape:${s.id}:inner:${key}`),result=ctx.safe(d.target);
      return `<div class="diameter-equation">${result.ok?displayMath(result.ast,d.symbol,E.Units.base(d.dimension).label):`<p class="inline-error">${esc(result.error)}</p>`}</div>`;
    }).join('');
    return `<section class="diameter-settings"><label class="field-label">Den oplyste diameter er<select class="source-select" data-action="diameter-basis" data-id="${esc(s.id)}" aria-label="Diameteren på ${esc(s.name)} er"><option value="inner" ${settings.basis==='inner'?'selected':''}>Indvendig diameter</option><option value="outer" ${settings.basis==='outer'?'selected':''}>Udvendig diameter</option></select></label><p class="field-note">Rumfang og flow bruger den indvendige diameter. Højde og længde skal fortsat være indvendige mål.${keys.length>1?' Valget gælder både D og d.':''}</p><details class="diameter-wall" data-detail-key="wall-${esc(s.id)}" ${settings.basis==='outer'?'open':''}><summary>${sloped?'Radial godstykkelse':'Godstykkelse til omregning'}</summary>${expressionEditor(settings.thickness,'length',['shapes',index,'diameter','thickness'],sloped?'t_radial'+s.ordinal:'t_væg'+s.ordinal,ctx)}<p class="field-note">${sloped?'På en skrå væg er dette tykkelsen i diameterretningen ved samme endeflade. T9 bruger pladetykkelsen vinkelret på væggen. Ved kendt væghældning kan du vælge formlen t_radial = t_plade · s / h.':'Som standard bruges samme fælles pladetykkelse som i T9. Har denne del en anden tykkelse, skal dens masse også beregnes med den tykkelse.'}</p><button class="text-button" data-action="edit-tank">Tilpas fælles pladetykkelse · T9</button></details><div class="diameter-derived"><span>Indvendige mål i formlerne</span>${lines}</div>${settings.basis==='outer'?'<p class="field-note">Den udvendige diameter skal være større end to gange den radiale godstykkelse.</p>':''}</section>`;
  }
  function renderTankInspector(ctx) {
    return `<div class="inspector-top"><span class="eyebrow">FÆLLES MÅL</span><div class="inspector-heading"><div class="inspector-icon">${symbolHtml('t_plade')}</div><div><h2>Pladetykkelse</h2><p>Bruges af T9 og diametermål</p></div></div></div><div class="inspector-content"><p class="inspector-intro">Vælg den fælles tykkelse som et kendt symbol eller en formel. Tankens egenvægt og de diametermål, der henviser hertil, følger samme kilde.</p>${expressionEditor(model.tank.thickness,'length',['tank','thickness'],'t_plade',ctx)}<p class="field-note">Vælg fx mm for tykkelsens symbol under Størrelserne i formlen. Omregningen til meter indsættes automatisk.</p><div class="inspector-actions"><button class="text-button" data-action="back-to-formula">Tilbage til formlen</button></div></div>`;
  }
  ...
  function renderInspector(ctx) {
    ...
    if(selected.type==='tank'){$('#inspector').innerHTML=renderTankInspector(ctx);return;}
  ...
  function renderLibrary() {
    $('#panel-library').innerHTML = `<div class="library-intro"><h2>Find formlen fra din opgave.</h2><p>Med T1–T39 fra skolens formelsamling. Vælg en grundformel, og brug Isolér en størrelse til at finde en anden ukendt. Forbind derefter størrelserne med figurer eller andre formler.</p></div><label class="formula-search">Søg i formelsamlingen<input type="search" data-action="search-formulas" value="${esc(libraryQuery)}" placeholder="Fx T9, tank, varme eller gearing" autocomplete="off"></label><div class="library-results"></div><details class="unit-guide"><summary>Enheder og omregninger</summary><p>Grundformlerne bruger SI. I formelbyggeren kan du vælge opgavens enheder under Størrelserne i formlen og vælge resultatets enhed. Omregningsfaktorerne indsættes automatisk; tilføj dem ikke igen.</p><div class="unit-table-wrap"><table><thead><tr><th scope="col">Størrelse</th><th scope="col">Fra → til</th><th scope="col">Omregning</th><th scope="col">Husk</th></tr></thead><tbody>${E.UNIT_GUIDE.map(row=>`<tr>${row.map(cell=>`<td>${esc(cell)}</td>`).join('')}</tr>`).join('')}</tbody></table></div></details><p class="school-source-note">Grundlag: ${esc(E.SCHOOL_SOURCE.title)} · ${esc(E.SCHOOL_SOURCE.school)} · ${esc(E.SCHOOL_SOURCE.date)}. Side- og T-numre henviser til jeres PDF. Der er både grundformler og omskrivninger. Forudsætninger og enhedstilpasninger står ved formlerne.</p>`;
    renderLibraryResults();
  }
  function renderLibraryResults() {
    const normalize=value=>String(value).normalize('NFKC').toLocaleLowerCase('da').replace(/[^\p{L}\p{N}]/gu,'');
    const query=libraryQuery.trim(),words=query.split(/\s+/).map(normalize).filter(Boolean);
    const formulas=Object.entries(E.BASE_FORMULAS).filter(([,f])=>{
      const haystack=normalize([f.name,f.group,f.equation,f.symbol,f.note,...(f.aliases||[]),...Object.values(f.args).flatMap(a=>[a.symbol,a.label]),schoolRef(f)].join(' '));
  ...
  function render(keepFocus = false) {
    ...
    if (focus) {
      const next = [...document.querySelectorAll('[data-action]')].find(e=>e.dataset.action===focus.action && e.dataset.id===focus.id && e.dataset.path===focus.path && e.dataset.face===focus.face && e.dataset.part===focus.part);
  ...
  function joinFields() {
    ...
    if(!hosts.length)return '<p>Indsæt en figur med en fri samleflade først, fx en kasse eller cylinder.</p>';
    ...
    if(!choices.some(c=>c.value===joinDraft.target))joinDraft.target=choices.find(c=>c.value===(face[1].kind==='rectangle'?'new:halfCylinder:top':'new:hemisphere:base'))?.value||choices[0]?.value||'';
    const endNote=face[1].kind==='rectangle'?'Til kassens bund kan du vælge en halvcylinder (rundbund), en V-bund med to trekantsender eller en pyramide med én spids. Længden følger kassens længde. Halvcylinderens diameter følger kassens bredde.':'En halvkugle er et kugleformet endestykke med højde D / 2. Samlinger forbinder hele, passende endeflader.';
    return `<div class="join-fields"><label>På hvilken figur?<select data-action="join-host">${hosts.map(s=>`<option value="${esc(s.id)}" ${s.id===host.id?'selected':''}>${esc(s.name)}</option>`).join('')}</select></label><label>På hvilken ende?<select data-action="join-face">${faces.map(([key,f])=>`<option value="${key}" ${key===joinDraft.face?'selected':''}>${esc(f.label)}</option>`).join('')}</select></label><label>Sæt denne figur på<select data-action="join-target">${choices.length?['Figurer i opsætningen','Indsæt og sammensæt'].map(group=>`<optgroup label="${group}">${choices.filter(c=>c.group===group).map(c=>`<option value="${esc(c.value)}" ${c.value===joinDraft.target?'selected':''}>${esc(c.label)}</option>`).join('')}</optgroup>`).join(''):'<option value="">Ingen ledige, passende endeflader</option>'}</select></label><div class="join-explanation">Samlefladerne får samme indvendige mål fra den første figur. Fladerne tælles ikke med i pladearealet. Når delene skilles ad, bruges igen hver figurs egne mål og åbne/lukkede valg.</div><p class="field-note">${esc(endNote)}</p></div>`;
  }
  ...
  function useAssembly(dimension) {
    ...
    if(!f){
      ...
      f=E.newFormula(newId('formula'),dimension==='volume'?'volumeSum':'areaSum');f.expression=E.assembly(dimension);f.symbol=dimension==='volume'?'V_samlet':'A_plade';f.name=dimension==='volume'?'Samlet rumfang':'Samlet pladeareal';model.formulas.push(f);save();
    }
    select('formula',f.id);
  }
  ...
  function formulaPicker() {
    ...
    openDialog('Hvad vil du finde?','<p>Vælg en grundformel. Under Isolér en størrelse kan du bagefter vælge en anden ukendt.</p><div class="dialog-formula-list">'+groups.map(group=>`<h3>${esc(group)}</h3>${Object.entries(E.BASE_FORMULAS).filter(([,f])=>f.group===group).map(([id,f])=>`<button class="dialog-formula-item" data-action="use-formula" data-formula="${id}"><span>${esc(formulaTitle(f))}</span><span>${esc(f.equation)}</span></button>`).join('')}`).join('')+'</div>');
  }
  ...
  async function copyFormula() {
    ...
    const quantities=E.variables(r.ast).map(v=>v.symbol+': '+(valueDisplay!=='units'&&v.inputValue!==undefined?v.inputValue.replace('.',',')+' ':'')+(v.reference?E.resultUnit(ctx.map.get(v.reference)):E.inputUnit(model,v)).label);
  ...
  const tutorialSteps = [
    ...
    {section:'FIGURER', title:'Indsæt de figurer, du skal bruge', target:'#shape-library', scene:'parts', body:'<p>Tryk på en figur i værktøjskassen for at indsætte den. Du kan fx bygge en tank af en <strong>cylinder</strong> og en <strong>kegle</strong>.</p><p>Til en kasse kan du vælge <strong>Halvcylinder</strong> som rundbund, <strong>V-bund</strong> med to trekantsender eller <strong>Pyramide</strong> med én spids. En halvkugle bruges som kuglespids på en cylinder.</p>'},
    ...
    {section:'RESULTAT', title:'Se den færdige formel', target:'#formula-preview', body:'<p><strong>Kort</strong> viser referencer som symboler. <strong>Udfoldet</strong> sætter delformlerne ind i hinanden, som vist her. Unødige parenteser fjernes automatisk. Nødvendige parenteser bevares fx omkring en sum, der skal ganges, en samlet nævner i kopieret tekst eller et udtryk under en potens.</p><p><strong>Kopiér formel</strong> tager den viste formel og de valgte enheder med. Du kan indsætte opgavens tal under <strong>Størrelserne i formlen</strong> og derefter regne udtrykket på din egen lommeregner.</p>'},
    {section:'OMSKRIVNING', title:'Find en anden størrelse', target:'.rearrange-control', body:'<p>Med <strong>Isolér en størrelse</strong> kan du vende grundformlen. Fra <strong>t = V / Qᵥ</strong> kan du fx vælge V eller Qᵥ. Ved <strong>v = π · D · n</strong> kan du vælge n og få <strong>n = v / (π · D)</strong>.</p><p>Omskrivningen oprettes som en ny formel, så dine eksisterende referencer bevares. Den tidligere venstreside bliver et kendt input. Kontrollér enheder og forudsætninger; enkelte ligninger har to rødder.</p>'},
  ...
  function prepareTutorialStep(step) {
    ...
    if(step.scene==='numbers'){
      model={version:5,inputUnits:{},inputValues:{'B:length':'50','L:length':'30'},title:'Introduktion · tal i formlen',tank:E.defaultTank(),shapes:[],connections:[],formulas:[E.newFormula('area','rectangleArea')]};
      activeId='area';
    }
  ...
  function startTutorial() {
    if (tutorial) return;
    tutorial = {model,activeId,selected,view,expanded,geometryView,previewPose:preview3D?.getPose()||previewPose,libraryQuery,valueDrafts,valueDisplay,resultVisible,
  ...
  document.addEventListener('click',event=>{
    ...
    if(a==='theme'){theme=theme==='dark'?'light':'dark';setTheme();try{localStorage.setItem(THEME,theme);}catch(_){}}
      ...
      save();render();
      [...document.querySelectorAll('[data-action="input-value"]')].find(el=>el.dataset.id===b.dataset.id)?.focus({preventScroll:true});
    }
    ...
    else if(a==='edit-tank')select('tank','tank',true);
    ...
    else if(a==='value-display'&&Object.hasOwn(VALUE_DISPLAYS,b.dataset.id)){
      valueDisplay=b.dataset.id;
      try{localStorage.setItem(DISPLAY,valueDisplay);}catch(_){/* Storage is optional. */}
      render(true);
    }
  ...
  document.addEventListener('change',event=>{
    ...
    if(a==='input-unit'||a==='input-unit-part'){
      const variable={symbol:el.dataset.symbol,dimension:el.dataset.dimension},key=E.Units.key(variable);
      ...
      if(f){f.resultUnit=(a==='result-unit-part'?E.Units.withPart(f.dimension,E.resultUnit(f).id,el.dataset.part,el.value):E.Units.get(f.dimension,el.value)).id;expanded=true;save();render(true);}
    }else if(a==='rearrange-formula'){
      if(!el.value)return;
      try{
        const id=newId('formula');model=E.rearrangeFormula(model,el.dataset.id,el.value,id);
        expanded=true;save();select('formula',id);notify('Omskrivningen er tilføjet. Vælg de kendte størrelser og deres enheder.');
  ...
  document.addEventListener('input',event=>{
    ...
    if(el.dataset.action==='input-value'){
      const key=el.dataset.id,value=E.parseInputValue(el.value);
      valueDrafts.set(key,el.value);model.inputValues||={};
      if(value===null||value==='')delete model.inputValues[key];else model.inputValues[key]=value;
      expanded=true;save();render(true);return;
    }
